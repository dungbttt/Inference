...
BlockArgs = collections.namedtuple('BlockArgs', ['num_repeat', 'kernel_size', 'stride', 'expand_ratio', 'input_filters', 'output_filters', 'se_ratio', 'id_skip', 'residual'])
...
class BlockDecoder(object):
    ...
    def _decode_block_string(block_string):
        ...
        return BlockArgs(
            ...
            residual=('residual' in block_string))
    ...
    def _encode_block_string(block):
        ...
        """
        args = [
            'residual' if block.residual else '',
...
def efficientnet(width_coefficient=None, depth_coefficient=None, image_size=None,
                 dropout_rate=0.2, drop_connect_rate=0.2, num_classes=8):
...
class ResidualBlock(nn.Module):
    def __init__(self, block_args, global_params, image_size=None):
        ...
        inp = self._block_args.input_filters  # number of input channels
        oup = self._block_args.output_filters  # number of output channels

        # First convolution layer
        Conv2d = get_same_padding_conv2d(image_size=image_size)
        self._conv1 = Conv2d(in_channels=inp, out_channels=oup, kernel_size=3, stride=1, bias=False)
        self._bn1 = nn.BatchNorm2d(num_features=oup, momentum=self._bn_mom, eps=self._bn_eps)
        image_size = calculate_output_image_size(image_size, 1)

        # Second convolution layer
        self._conv2 = Conv2d(in_channels=oup, out_channels=oup, kernel_size=3, stride=1, bias=False)
        self._bn2 = nn.BatchNorm2d(num_features=oup, momentum=self._bn_mom, eps=self._bn_eps)
        image_size = calculate_output_image_size(image_size, 1)

        # Skip connection (if applicable)
        if inp != oup:
            self._skip_conv = Conv2d(in_channels=inp, out_channels=oup, kernel_size=1, stride=1, bias=False)
            self._skip_bn = nn.BatchNorm2d(num_features=oup, momentum=self._bn_mom, eps=self._bn_eps)
        else:
    ...
    def forward(self, inputs):
        ...
        if self._skip_conv is not None:
            skip = self._skip_conv(inputs)
            skip = self._skip_bn(skip)
        else:
            skip = inputs
        x += skip
        x = self._swish(x)
...
class EfficientNet(nn.Module):
    def __init__(self, blocks_args=None, global_params=None):
        ...
        for block_args in self._blocks_args:
            ...
            if hasattr(block_args, 'block_type') and block_args.block_type == 'residual':
                self._blocks.append(ResidualBlock(block_args, self._global_params, image_size=image_size))
            else:
    ...
    def from_pretrained(cls, model_name, weights_path=None, advprop=False, 
        ...
        load_pretrained_weights(model, model_name, weights_path=weights_path, load_fc=False, advprop=advprop)  # Do not load final layer weights

        # Replace the final fully connected layer
        model.classifier = nn.Linear(model._fc.in_features, num_classes)

        # Initialize the new final layer (optional, if needed)
...
if dataset in ['SID', 'SMID', 'SDSD_indoor', 'SDSD_outdoor']:
    ...
    single_image_path = '/kaggle/input/colors/data lab/data lab/chen/chen/black/black_0.jpg'  
